...
"no, grug will not build that feature"

"no, grug will not build that abstraction"

"no, grug will not put water on body every day or drink less black think juice stop ask"
...
"ok, grug will build that feature"
...
mind like butterfly at times often forget what even supposed to do and move on or get fired.  grug see happen many time.
...
demon internally, like trapped in crystal.  grug quite satisfied when complexity demon trapped properly, grug hate
complexity demon

grug watch as cut points emerge and slowly refactor, with code base taking shape over time.  no hard and fast rule for
this: grug know it when grug see it, just take time to build skill.  sometimes grug go too early and get abstractions
wrong, so grug bias towards waiting

big brain developers often not like this at all and invent many abstractions at start of project.  grug tempted to reach
for club and scream "you no maintain!  you move on to next architecture committee!" but grug learn control passions, major
difference between grug and animals.  instead grug try to limit damage of big brain developer by giving them thing like
UML diagram (not hurt code, throw away anyway) or by demanding working demo ASAP.

working demo especially good trick: force big brain make something to actually talk about and code to look at that do
thing, will help big brain see reality on ground more quickly.  big brain have big brain!  need only be harness for good
and not in service of complexity demon on accident.  best grug brain able to herd multiple big brain in right direction and produce
many complexity demon trap crystals.
...
grug have love/hate relationship with test: test save grug many, many time and grug love and respect test.  but also
test shamans exist and make test idol, demand "test first" before grug even write code or have any idea what
grug doing!  how grug test what grug not even understand himself?

"Oh, don't worry: the tests will show you what you need to do."

grug once again find self slowly reaching for club

grug prefer write tests after prototype phase, when code has begun to firm up.  grug must be disciplined!  easy to move
on and not write tests once code "work on grugs machine", but this bad: no guarantee work on other machine and no guarantee
work in future on grug machine.  test shaman have point on importance of test, even if test shaman never complete a productive
feature in life and talk only about test all time

grug experience that ideal tests are not unit test or end to end tests, but in-between.

unit tests fine, ok, but break and make refactor hard as API change and, frankly, not too useful many bugs anyway.
...
in-between tests, grug hear call "integration tests" sometimes, are sweet spot according to grug: high level enough to test
correctness of system, low level enough that, with debugger, easy enough to see what breaking

grug prefer some unit tests, many more integration test as cut points emerge, and small, curated end-to-end test suite
that is kept working religiously, focus on most common UI features and a few important edge cases.

this ideal set of test.  you may not like it, but this peak testing.
...
grug wonder why take hardest problem, factoring system correctly, and introduce network call too
...
brain, really makes grug think

good debugger worth weight in shiney rocks, in fact more: when faced with bug grug would often trade all shiney rock and
perhaps few children for good debugger and anyway debugger no weigh anything.  grug always recommend new programmer 
learn available debugger very deeply, features like conditional break points, etc.

never not be improving tooling

# Type Systems

grug like type systems make programming easier.  for grug, type systems most value when grug hit period on keyboard and
list of things grug can do pop up.  this 90% of value of type system.

beware big brains here.  certain type big brain think in type systems and talk in lemmas, very dangerous.  

big brain type system shaman often say type correctness main point type system, but grug note big brain type system 
shaman not ship any code.  grug suppose code never shipped is correct, in some sense, but not really what grug mean
when say correct.

also danger abstraction too high, and big brain code become platonic astral projection of generic turing model of 
computation into code base.  grug confused and agree some level very elegant but also very hard do anything like 
record club inventory for Grug Inc.  generics especially dangerous here, grug limit generics to container classes for 
most part where most value add.

most value type system come hit dot see what can do, never forget.

# Expression Complexity

grug once like to minimize lines of code as much as possible.  write code like
...
over time grug learn this hard debug, and prefer write like so:
...
understand conditional expression!  oof!  EASIER DEBUG!  braaaaaaaaah!"

definitely easier debug and once club fight end and young grug think a bit, they realize grug right

grug still catch grug writing code like first example and often regret, so grug not judge

# Closures

grug like closures for right job and that job usually abstracting operation of collection of objects.

grug warn closures like type systems and generics: small amount go long way, but easy spoil things too much use

javascript developers TODO
...
grug do there?!?" so not pursue.  turn out logging _very_ important to google so of course best programmers work on it, grug!  don't
be such grug brain, grug!  oh well, grug end up at good company anyway and rob pike dress habit increasingly erratic, so 
all work out in end, but point stand: logging very important!
...
* if possible make logging level dynamically controlled, so grug can turn on/off when need to debug issue
* if possible make logging level per user, so can debug specific user issue

last two points are especially handy when fighting bugs in production systems

unfortunately log libraries often very complex (java, what are you doing?) but is worth investing time in getting 
...
remote job worker queues, where jobs not interdepend

optimistic concurrency seem work well for web anyway

occasionally grug reach for thread local variable, usually when writing framework code

some language have good concurrent data structures, like java's ConcurrentHashMap, but still need careful grug work
to get right

grug has never used erlang, hear good things, but language look wierd

# APIs

grug love good apis.  good apis don't make grug think to much.

unfortunately, many apis very bad, make grug think quite a bit.  this happen for many reasons, here two:

* API creators think in terms of the implementation of the API, rather than in terms of the use of an API
...
`write()` or `sort()` or whatever.  but big brain api developers say "not so fast, grug!  is that file *open for write*?
did you define a *Comparator* for that sort?"

grug find self reaching for club again, not care about that stuff right now, just want sort and write file!

grug recognize that big brain api designer have point and that _sometimes_ these things matter, but often do not.
big brain api developers better if design for simple, common cases with simple code, and make complex cases possible
with more complex code

grug call this "layering" apis: two or three different apis at different level complexity for needs

also, put api on thing instead of elsewhere.  java worst at this!  grug want filter list.  "Did you convert it to a stream?"
fine, grug convert to stream.  "OK, now you can filter."  OK, but need return list!  now stream!  "Did you collect it in
a list?"

grug reaching for club again

put common thing like `filter()` on list and make return list big brain api developer!

# Front End Development

some non-grugs, when faced with web development say: "I know, I'll split my front end and back end codebase up and use
a hot new SPA library talking to a GraphQL JSON API."

now you have two complexity demon spirits and, what is worse, front end complexity demon spirit even more powerful and 
have hold on entire industry as far as grug can tell.  back end developers try keep things simple and can work ok, but
front end get very complex very quickly and introduce lots of code.  

even when just need put form into database!  everyone  do this now!  grug not sure why except maybe facebook and google
say so, but that not seem very good reason to grug
...
keep complexity low, simple HTML, avoid lots JavaScript (natural ether of complexity demon!)

maybe they work for you, but no job postings, sorry

# Fads

grug note lots of fads in development, especially front end development.  back end better more boring because all bad
ideas have been tried at this point maybe.  still trying all bad ideas in front end development maybe.
...
a lot of time wasted on recycled bad ideas, and lots of complexity demon power come from jamming "new hotness" into
code base

very good if senior grug willing to say publicly: "hmmm, this too complex", make it ok for junior grugs to admit that as well
so that complexity demon has no power over them!  make thinking face and look big brained when saying and be prepared
for big brain or, worse, *thinks* is big brain to make fun.  be strong!  club useful here, but sense of humor and especially
...
remember young grug: complexity *very*, *very* bad